          ...
          <v-form ref="form" lazy-validation>
            <v-row>
              <v-col md="12" sm="12" cols="12">
                <v-textarea outlined label="รายละเอียดการรักษา" v-model="treatmens_detail" :rules="[(v) => !!v || '']"
                  required></v-textarea>
              </v-col>
            </v-row>
            <v-row class="mt-2">
    ...
    async fn_detaailTreat(item) {
      ...
      await axios
        ...
        .then(async (res) => {
          this.Treatment = res.data.data;
          await axios
            .get(`${process.env.api_url}/Treatment_con/get_img?id_customer=${this.$route.query.idcus}&id_type=${item.ID_treat}&id_rendezvous=`, {
              headers: {
                "Content-Type": "application/json",
              },
            })
